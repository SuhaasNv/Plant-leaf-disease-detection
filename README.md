...
- **Railway:** `API_KEY`, `MODEL_URL`, `CORS_ORIGINS`
- **Vercel:** `API_URL` (Railway URL), `API_KEY` (same as Railway) — no `NEXT_PUBLIC_` prefix
...
- Env vars: `API_URL` = Railway URL, `API_KEY` = same secret (server-side only)
