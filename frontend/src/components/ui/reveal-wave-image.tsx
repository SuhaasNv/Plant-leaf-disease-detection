...
const fragmentShader = `
  ...
  void main() {
    ...
    if (uMouseActive > 0.01) {
      ...
      float ripple = sin(dist * uWaveFrequency * 5.0 - uTime * uWaveSpeed) * uWaveAmplitude * 0.05 * mouseInfluence * uMouseActive;
    ...
    float revealAmount = (1.0 - smoothstep(uRevealRadius * (1.0 - uRevealSoftness), uRevealRadius, revealDist)) * uMouseActive;
    gl_FragColor = vec4(mix(bwColor, color.rgb, revealAmount), color.a);
  }
`;
...
function ImagePlane({
  ...
  useFrame((state) => {
    ...
    mouseActiveRef.current += ((isMouseInCanvas ? 1 : 0) - mouseActiveRef.current) * 0.08;
    mat.uniforms.uMouseActive.value = mouseActiveRef.current;
    if (hasEnteredRef.current) {
      mat.uniforms.uMouse.value.set((pointer.x + 1) / 2, (pointer.y + 1) / 2);
    }
  });
